...
def save_image_batch(imgs, output, col=None, size=None, pack=True,targets=None):
    ...
    if pack:
        imgs = pack_images( imgs, col=col )
        imgs = imgs.transpose( 1, 2, 0 ).squeeze() 
        ...
        for idx, img in enumerate(imgs):
            ...
            if targets is None:
                ...
                os.makedirs(os.path.join(output.strip(file_name),str(int(targets[idx]))), exist_ok=True)
                output_filename=os.path.join(output.strip(file_name),str(int(targets[idx])),file_name.strip('.png'))
                img.save(output_filename+'-%d.png'%(idx))
...
class ImagePool(object):
    ...
    def add(self, imgs, targets=None):
        if targets is not None:
            save_image_batch(imgs, os.path.join( self.root, "%d.png"%(self._idx) ), pack=False,targets=targets)
        else:
...
class AlignHead(nn.Module):
    """align the dimension of feature maps"""
    def __init__(self, s_shape, t_shape):
        ...
        self.s_H = s_H
        self.t_H = t_H
        num_input_channels = s_C
        num_target_channels = t_C
    ...
    def forward(self, x):
        if self.s_H != self.t_H:
            x = nn.functional.adaptive_avg_pool2d(x, (self.t_H, self.t_H))
        x = self.regressor(x)
        return x