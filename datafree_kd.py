...
# Data Free
parser.add_argument('--method', required=True, choices=['zskt', 'dafl', 'adi', 'cmi', 'csd'])
...
parser.add_argument('--csd', default=0, type=float, help='scaling factor for csd loss')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.method == 'csd':
        csd_evaluator = datafree.evaluators.csd_classification_evaluator(val_loader)
    ...
    if args.method=='adi':
        ...
        synthesizer = datafree.synthesis.CSDSynthesizer(teacher=teacher, student=student, generator=generator,
                ...
                adv=args.adv,csd=args.csd, bn=args.bn, oh=args.oh,save_dir=args.save_dir, transform=ori_dataset.transform, dataset=args.dataset,
                normalizer=args.normalizer, device=args.gpu)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.method == 'csd':
            ...
            csd_eval_results =csd_evaluator(student, synthesizer.head, device=args.gpu)
            (csd_acc1, ), csd_val_loss = csd_eval_results['Acc'], csd_eval_results['Loss']
            tb_writer.add_scalar('AUX/Acc', csd_acc1, args.current_epoch)
            tb_writer.add_scalar('AUX/Loss', csd_val_loss, args.current_epoch)
        eval_results = evaluator(student, device=args.gpu)
